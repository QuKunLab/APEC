...
#
opts = OptionParser()
usage = "Organized prepared data\nusage: %prog -m matrix -p peak --ref ref --fa chr.fa --bg bg.txt --meme motifs.meme --np 4"
...
#
#
top_peaks = options.p + '/top_peaks.bed'
print '!!!!!!  annotate peaks  !!!!!!'
annotate_output = options.p + '/annotate_output.bed'
annotate_peak = options.p + '/annotate_peak.bed'
#
os.popen("annotatePeaks.pl " + top_peaks + " " + options.ref + " -size given > " + annotate_output)
temp01_file = options.p + '/temp01.bed'
...
trans_bias = options.p + 'transposase_bias_filtered.bed'
temp02_file = options.p + 'temp02.bed'
temp03_file = options.p + 'temp03.bed'
...
# motif annotation
TFmatrix_file = options.m + '/motif_filtered.csv'
